...
class LLMGatewayOpenAI(BaseModel):
    CHAT_ENGINE_MODELS = [
        "gpt-3.5-turbo",
        "gpt-3.5-turbo-0301",
        "gpt-3.5-turbo-0613",
    ...
    def __init__(self, config: AutolabelConfig, cache: BaseCache = None) -> None:
        ...
        try:
            self.llm = self._create_llm_gw_connection()
        except:
            raise RuntimeError(
                "Something went wrong with connecting to Salesforce LLM Gateway."
            )        

        
    def _create_llm_gw_connection(self):
        falcon_instance = os.getenv("FALCON_INSTANCE")
        ssl_ca_cert = os.getenv("SSL_CA_CERT")
        access_token = os.getenv("LLM_API_KEY")
        url = f"https://bot-svc-llm.sfproxy.einstein.${falcon_instance}.aws.sfdc.cl"
        org_id = "00DSG00000061ur2AA"
